...
const classCode = () => String(process.env.CLASS_CODE || '').trim();
const instructorCode = () => String(process.env.INSTRUCTOR_CODE || '').trim();
...
function validateAuthEnvOrExit() {
  ...
  if (realSignIn) {
    ...
    problems.push('       CLASS_CODE="Builders Club"');
    problems.push('       INSTRUCTOR_CODE=<a private code only you know>');
    problems.push('       SESSION_SECRET=<openssl rand -hex 32>');
    problems.push('');
    problems.push('  b) School Microsoft accounts — needs an Entra app registration:');
    problems.push('       MS_TENANT_ID, MS_CLIENT_ID, MS_CLIENT_SECRET, SESSION_SECRET, PUBLIC_BASE_URL');
    problems.push('');
