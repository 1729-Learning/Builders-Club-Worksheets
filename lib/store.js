...
function readProfile(oid) { return readJSON(profileFile(oid), null) || {}; }
...
/* ---------------------------------------------------------------- class archive
   ...
   Every student's work in one file, downloaded from the running site. The
   platform's own volume backups are the first line of defence, but they live on
   the platform, they are a paid feature on some plans, and wiping a volume takes
   its backups with it. This is the copy an instructor can keep somewhere else
   entirely, and restore into a fresh service.

   Snapshots (backups/) and AI usage counters are deliberately left out: the
   first would dwarf the work itself, and the second is a rate limit, not
   something worth carrying between disks. */

const ARCHIVE_KIND = 'builders-club-class-archive';

function exportArchive() {
  let dirs = [];
  try { dirs = fs.readdirSync(STUDENTS_DIR).filter(isOid); } catch { /* nobody has signed in yet */ }
  const students = dirs.map(oid => ({
    oid,
    profile: readJSON(path.join(STUDENTS_DIR, oid, 'profile.json'), null) || {},
    state: readJSON(path.join(STUDENTS_DIR, oid, 'state.json'), null) || freshState(),
    settings: readJSON(path.join(STUDENTS_DIR, oid, 'settings.json'), null) || {},
  }));
  return { kind: ARCHIVE_KIND, version: 1, exportedAt: Date.now(), students };
}

/* Restoring is additive: it writes the students the archive names and leaves
   everyone else alone, so a stale archive can never delete a student who has
   signed in since. Each write goes through writeState, which snapshots whatever
   is on disk first — restoring onto live work is itself undoable, per student. */
function importArchive(archive) {
  if (!archive || typeof archive !== 'object' || archive.kind !== ARCHIVE_KIND || !Array.isArray(archive.students)) {
    return { ok: false, error: 'That file is not a class archive.' };
  }
  let restored = 0, skipped = 0;
  for (const entry of archive.students) {
    const oid = entry && entry.oid;
    const state = entry && entry.state;
    // Same shape check the save route makes: anything that isn't a worksheet
    // state would overwrite real work with nonsense.
    if (!isOid(oid) || !state || typeof state !== 'object' || typeof state.steps !== 'object' || state.steps === null) {
      skipped++;
      continue;
    }
    writeState(oid, state);
    if (entry.settings && typeof entry.settings === 'object') writeSettings(oid, entry.settings);
    // A live profile is more current than an archived one, so it wins. On a
    // fresh disk there is none, and the archive is what gives the roster names.
    if (entry.profile && typeof entry.profile === 'object' && !fs.existsSync(profileFile(oid))) {
      writeJSON(profileFile(oid), { ...entry.profile, oid: assertOid(oid) });
    }
    restored++;
  }
...
module.exports = {
  ...
  readProfile, upsertProfile, touchLastSeen,
